...
umi_tools extract --stdin=${data_set} --bc-pattern=NNNNNNNNNNN --log=${out_dir}${srr}.extract.log --stdout ${processed_data_set}

alignment_bam="${out_dir}${srr}.alignment.bam"

# bowtie2-build ${ref_genome} $ref_index

bowtie2 -x $ref_index -U ${processed_data_set} --local -p 40 | samtools view -bS > ${alignment_bam}

sorted_alignment_bam="${out_dir}sorted.${srr}.alignment.bam"
samtools sort ${alignment_bam} -o ${sorted_alignment_bam}
samtools index ${sorted_alignment_bam}

dedup_bam="${out_dir}${srr}.dedup.bam"
dedup_log="${out_dir}${srr}.deduplicated"
umi_tools dedup -I ${sorted_alignment_bam} --output-stats=${dedup_log} -S ${dedup_bam}  

groups_log="${out_dir}${srr}.groups.tsv"
mapped_group_bam="${out_dir}${srr}.mapped_grouped.bam"
